...
function t() { return i18n[currentLang]; } function setLanguage(lang, state) { currentLang = (lang && i18n[lang]) ? lang : 'tr'; if (els.langSelect) els.langSelect.value = currentLang; if (els.setsLabel) els.setsLabel.textContent = i18n[currentLang].setsLabel; if (els.langLabel) els.langLabel.textContent = i18n[currentLang].langLabel; if (els.pauseBtn) els.pauseBtn.textContent = i18n[currentLang].pause; if (els.resetBtn) els.resetBtn.textContent = i18n[currentLang].reset; if (state) { if (state.isPaused) { if (state.phase === 'idle') setStartButtonLabel('start'); else setStartButtonLabel('resume'); } } else { setStartButtonLabel('start'); } if (state) { setPhase(state.phase); if (state.phase === 'finished') { els.setInfo.textContent = i18n[currentLang].finishedInfo(state.totalSets); } else { updateSetInfo(state.currentSet, state.totalSets, state.phase); } }
